...
vrtql_url vrtql_url_parse(const char* url)
{
    ...
    if (scheme_end != NULL)
    {
        ...
        parts.scheme = (char*)malloc(scheme_length + 1);
        ...
        strncpy(parts.scheme, url, scheme_length);
        parts.scheme[scheme_length] = '\0';
        url = scheme_end + 3;
    }
    ...
    {
        size_t fragment_length = strlen(fragment_start + 1);
        parts.fragment = (char*)malloc(fragment_length + 1);
        ...
        strcpy(parts.fragment, fragment_start + 1);

        // Truncate the URL at the fragment
        url = strndup(url, fragment_start - url);
    }
    ...
    if (query_start != NULL)
    {
        size_t query_length = strlen(query_start + 1);
        parts.query = (char*)malloc(query_length + 1);
        ...
        strcpy(parts.query, query_start + 1);

        // Truncate the URL at the query
        url = strndup(url, query_start - url);
    }

    // Find the path, host, and port
    ...
    if (path_start != NULL)
    {
        ...
        if (port_start != NULL)
        {
            size_t port_length = strlen(port_start + 1);
            parts.port = (char*)malloc(port_length + 1);
            ...
            strcpy(parts.port, port_start + 1);

            // Truncate the host at the port
            char* prev = host;
            host = strndup(host, port_start - host);
            free(prev);
        }
    ...
    {
        // No path specified, consider the entire URL as the host
